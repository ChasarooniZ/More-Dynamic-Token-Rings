...
export async function createCustomTokenRingDialog() {
  const defaultSettings = {
    quality: 80,
    thickness:
      game.settings.get(MODULE_ID, "custom-ring.ring-thickness") || 0.1,
    innerRing:
      game.settings.get(MODULE_ID, "custom-ring.color-band.start") || 0,
    outerRing: game.settings.get(MODULE_ID, "custom-ring.color-band.end") || 1,
    ringColor:
      game.settings.get(MODULE_ID, "custom-ring.color-band.color") || "#ffffff",
  };
  const IMG_BASE_URL =
    "https://raw.githubusercontent.com/ChasarooniZ/More-Dynamic-Token-Rings/main/previews/tutorial/";
  const IMGS = {
  ...
  <form>
    ...
    buttons: [
      {
        ...
        callback: async (html) => {
          ...
          await processAndSaveConfigJSON(
            thickness,
            innerRing,
            outerRing,
            ringColor
    ...
    render: (html) => {
      ...
      if (!checkKofi(game.settings.get(MODULE_ID, "custom-ring.kofi-code"))) {
        const header = html
          .closest(".dialog")
          .find("a.header-button.control.close");
